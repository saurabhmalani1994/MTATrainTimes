...
Temperature Cycling Feature (33% F, 34% C, 33% F)
...
- Temperature cycling in weather display (F → C → F)
"""
...
class DisplayManager:
    ...
    TEMP_CYCLE_FRAMES = 150  # 90 frames = 3 seconds at 30 FPS
    ...
    def _should_show_celsius(self):
        """
        Determine if we should show Celsius based on frame count
        Cycle: F (33%) → C (34%) → F (33%)
    ...
    def render_weather(self, weather_data):
        ...
        Temperature displays cycle: F (33%) → C (34%) → F (33%)
        ...
        try:
            ...
            # ============================================================================
            # ROW 3: Current temp and real feel - DYNAMIC (F/C cycling)
            # ============================================================================
            font_temp = self.fonts['weather_temp']
            y_row3 = y_row2 + row_height + 1
            
            # Determine if we should show Celsius
            show_celsius = self._should_show_celsius()
            
            if weather_data.temperature is not None:
                if show_celsius:
                    # Convert to Celsius
                    temp_c = self._fahrenheit_to_celsius(weather_data.temperature)
                    if weather_data.real_feel is not None:
                        real_feel_c = self._fahrenheit_to_celsius(weather_data.real_feel)
                        temp_text = f"{temp_c}°C ({real_feel_c}°C)"
                    else:
                        temp_text = f"{temp_c}°C"
                else:
                    # Show Fahrenheit (original)
                    if weather_data.real_feel is not None:
                        temp_text = f"{weather_data.temperature}°F ({weather_data.real_feel}°F)"
                    else:
                        temp_text = f"{weather_data.temperature}°F"
            else:
                temp_text = "-- °" + ("C" if show_celsius else "F")
            ...
            # ============================================================================
            # ROW 4: High and Low temps - DYNAMIC (F/C cycling)
            # ============================================================================
            ...
            if weather_data.high_temp is not None and weather_data.low_temp is not None:
                if show_celsius:
                    # Convert to Celsius
                    high_c = self._fahrenheit_to_celsius(weather_data.high_temp)
                    low_c = self._fahrenheit_to_celsius(weather_data.low_temp)
                    hi_lo_text = f"H:{high_c}° L:{low_c}°"
                else:
                    # Show Fahrenheit (original)
